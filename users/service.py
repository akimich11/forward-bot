...
class MessageService(db.ConnectionMixin):
    ...
    def save_message(cls, message: Message, cursor):
        cursor.execute("INSERT INTO messages (user_id, message_id, sent_at) VALUES (%s, %s, to_timestamp(%s))",
                       (message.chat.id,
                        message.id,
                        message.date))
    ...
    def get_greeting_messages(cls, user_id, cursor):
        cursor.execute("SELECT message_id FROM messages WHERE user_id=(%s) ORDER BY sent_at", (user_id,))

    @classmethod
    @db.fetch(return_type='all_tuples')
    def get_users_who_sent_greetings(cls, cursor):
        cursor.execute("SELECT DISTINCT u.id, name FROM messages JOIN users u ON u.id = messages.user_id ")
...
class UserService(db.ConnectionMixin):
    ...
    def create_user(cls, user_id, cursor):
        cursor.execute("INSERT INTO users (id, name) VALUES (%s, null) ON CONFLICT (id) DO "
